...
def apply_filters_long(state: supervisely.app.StateJson):
    ...
    id_col_index = table_widgets.images_table.get_json_data()["table_data"][
        "columns"
    ].index("id")
...
def remove_all_filters_clicked(
    state: supervisely.app.StateJson = Depends(supervisely.app.StateJson.from_request),
):
    StateJson()["selected_filters"] = []
    StateJson()["current_preset"] = DataJson()["available_presets"][0][
        "name"
    ]  # All images
    StateJson().send_changes()
...
def selected_preset_changed(
    ...
    if current_preset_filters is None:
        supervisely.logger.warn(
            f"Not found filters for preset: {state['current_preset']}"
        )
...
@g.app.post("/set_images_limit/")
def input_limit_changed(
    state: supervisely.app.StateJson = Depends(supervisely.app.StateJson.from_request),
):
...
def selected_filter_changed(
    ...
    if selected_filter is None:
        supervisely.logger.warn(
            f"Not found filter with name: {state['selected_filter']}"
        )
...
@card_widgets.reselect_filters_button.add_route(
    app=g.app, route=ElementButton.Routes.BUTTON_CLICKED
)
def reselect_filters_button_clicked(
    state: supervisely.app.StateJson = Depends(supervisely.app.StateJson.from_request),
):
    StateJson()["selected_filters"] = []
    StateJson()["current_preset"] = DataJson()["available_presets"][0][
...
def selected_filter_changed(
    ...
    StateJson()["selected_filters"][state["filter_to_change"]]["data"][
        "valueType"
    ] = tag_data["value_type"]
